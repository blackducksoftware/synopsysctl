...
binary: clean ${OUTDIR} 
	$(foreach p,${PLATFORM}, \
		...
		if [ "$(p)" = "${WINDOWS}" ]; then \
			docker run --rm -e CGO_ENABLED=0 -e GOOS=$(p) -e GOARCH=amd64 -v "${CURRENT_DIR}":/go/src/github.com/blackducksoftware/synopsysctl -w /go/src/github.com/blackducksoftware/synopsysctl/cmd/synopsysctl golang:1.19 go build -ldflags "-X main.version=${TAG}" -o /go/src/github.com/blackducksoftware/synopsysctl/${OUTDIR}/$(p)/synopsysctl.exe; \
		else \
			docker run --rm -e CGO_ENABLED=0 -e GOOS=$(p) -e GOARCH=amd64 -v "${CURRENT_DIR}":/go/src/github.com/blackducksoftware/synopsysctl -w /go/src/github.com/blackducksoftware/synopsysctl/cmd/synopsysctl golang:1.19 go build -ldflags "-X main.version=${TAG}" -o /go/src/github.com/blackducksoftware/synopsysctl/${OUTDIR}/$(p)/synopsysctl; \
		fi && \
...
package:
	$(foreach p,${PLATFORM}, \
		...
		cd ${OUTDIR}/$(p) && \
		if [ "$(p)" = "${LINUX}" ]; then \
			tar -zcvf synopsysctl-$(p)-amd64-$(TAG).tar.gz synopsysctl && mv synopsysctl-$(p)-amd64-$(TAG).tar.gz .. && cd .. && $(SHA_SUM_CMD) synopsysctl-$(p)-amd64-$(TAG).tar.gz >> CHECKSUM && rm -rf $(p); \
		elif [ "$(p)" = "${WINDOWS}" ]; then \
			zip synopsysctl-$(p)-amd64-$(TAG).zip synopsysctl.exe && mv synopsysctl-$(p)-amd64-$(TAG).zip .. && cd .. && $(SHA_SUM_CMD) synopsysctl-$(p)-amd64-$(TAG).zip >> CHECKSUM && rm -rf $(p); \
		else \
			zip synopsysctl-$(p)-amd64-$(TAG).zip synopsysctl && mv synopsysctl-$(p)-amd64-$(TAG).zip .. && cd .. && $(SHA_SUM_CMD) synopsysctl-$(p)-amd64-$(TAG).zip >> CHECKSUM && rm -rf $(p); \
		fi && \
